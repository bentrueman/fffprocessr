...
Integrate the entire fractogram for comparison with an external measurement using the function `integrate_peak()`. You'll have to supply the injection volume (L) and the flowrate (L/min) to get a concentration in the expected units. Compare integrated peak areas with directly-quantified concentrations (no FFF) using `load_direct_quant()`, which reads and cleans ICP-MS data files generated using the iCAP-RQ.

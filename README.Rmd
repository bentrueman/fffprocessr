...
Use the `integrate_peak()` function to assign a concentration estimate to each peak. You'll have to supply the injection volume (L) and the flowrate (L/min) to get a concentration in the expected units. 
...
where *beta~0~* and *beta~1~* are the intercept and slope of the linear regression. This is the Zimm model; see @kammer_field_flow_2005 (and references therein) for details (the PostNova AF2000 software manual will also be helpful).  
...
```{r rg, fig.height=3.5, warning=FALSE, echo=FALSE}
mals_rg %>% 
  filter(param == "ls90") %>% 
  pivot_longer(c(rg_zimm, conc)) %>% 
